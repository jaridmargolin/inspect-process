...
module.exports = function (cmd, options) {
  ...
  return findOpenPort()
    .then((port) => inspectProcess(cmd, options, port));
};
...
const inspectProcess = function (cmd, options, port) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    proc.stderr.on('data', (data) => {
      ...
      if (isListening) {
        devtools.open(dataStr.substring(dataStr.indexOf('chrome-devtools')));

      } else if (isComplete) {
        devtools.close();
      }

      if ((!isListening && !isComplete && !isAttached) || inspectOptions['verbose']) {
